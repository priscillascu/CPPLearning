...
函数有个重要概念，**实参和形参**，很好理解，形参就是一个形式上的参数，即定义函数时，函数名后跟的参数，而实参则是调用函数时，所使用的参数。定义函数`void Func(int a)`，其中参数`a`就是形参。调用函数时，`Func(b);`，其中`b`就是实参。函数运行时，将实参**复制**了一份给形参，函数改变形参，对实参没有影响，这种方式也叫做**值传递**，占用空间较多。函数的参数和在调用函数时写入的参数之间有个`=`的关系，例如：
```
void Fun1(int &a);
...
Fun1(b);
```
此时，存在一个`int &a = b`的关系。判断函数调用时传入的参数是否符合要求，可以直接将函数定义时的参数与传入的参数之间**添加等号**，看看是否成立。
...
### 多态
多态是C++中非常重要的一个概念，之前我们就已经接触到了多态中的一种：静态多态，即函数重载、运算符重载。多态分为：
- 静态多态：函数重载、运算符重载，编译阶段确定函数地址，早绑定
- 动态多态：派生类和虚函数实现运行时多态，运行阶段确定函数地址，晚绑定
...
将子类传入参数为父类的引用的函数，即父类的引用指向了子类的对象。C++允许父类和子类之间的类型转换，父类的引用、指针可以指向子类。

**重写**和**重载**的区别在于，重写是函数除了实现部分以外，其余都一摸一样；而重载则只是函数名一样。

当我们使用继承定义了一个子类，该子类中有父类的函数的**重写**时，如果我们通过父类的指针或者引用，指向了子类对象，如：
```
void Func(Base &base1)  // 定义一个参数为父类引用的函数
{
    base1.Member();
}

Func(Child child1);  // 给该函数传入子类对象
```
此时，该函数是调用子类的重写函数呢？还是父类的原函数呢？这就需要看父类的原函数的地址是否进行了早绑定：在编译阶段就确定了函数的地址，晚绑定：函数的地址不能确定，需要根据传入的对象来确定是否调用子类的同名函数。

如果要实现晚绑定，使得将子类对象传给参数为父类指针、引用的函数，可以调用子类的重写函数，就需要将父类的原函数声明为一个**虚函数**`virtual`：
```
class Base
{
private:
...
```
**原理：** 虚函数在类内部是用一个虚指针来指向一个虚函数表，虚函数表记录虚函数的地址，当子类发生父类虚函数重写时，会用子类的重写函数替换父类的虚函数表

因为父类中的虚函数通常是含无意义的，不会被父类调用，此时可以将父类的虚函数改为**纯虚函数**：
```
virtual 返回值类型 函数名() = 0；
```
不需要写函数的具体实现和{}了。

当类中有了纯虚函数，这个类就是**抽象类**，此时抽象类是无法实例化对象，抽象类的子类必须重写父类的纯虚函数，否则子类也是抽象类：
```
class Base
{
public:
    virtual void Func() = 0;  // 定义一个纯虚函数，此时Base类为抽象类，不能实例化对象
};

// Base b;  // 报错，无法使用抽象类实例化对象

class Child : public Base
{
public:
    // 抽象类的子类重写父类的纯虚函数，否则也是抽象类
    void Func()
    {
        ....
    }
...
```
### 虚析构和纯虚析构
当父类指针、引用了一个子类对象，此时调用父类的析构函数是无法析构子类的，需要将父类的析构函数声明成一个**虚析构**：
```
class Base
{
private:
...
```
上述代码中，若父类`Base`的析构函数不声明成虚析构，则无法释放子类所`new`的堆区，导致内存泄漏：
```
void test1()
{
    // 定义一个指针子类的父类对象
...
```
纯虚析构的语法很简单，需要注意的是，除了声明之外，还需要写一个具体实现，即{}，有纯虚析构则是抽象类：
```
virtual ~类名() = 0;
...
```
虚析构和纯虚析构是用来解决通过父类指针来释放子类对象，如果子类**没有堆区数据**，那么是可以**不写虚析构**的。

## 模板
模板是C++**泛型编程**的一个重要工具，利用模板，可以在不知道数据类型的情况下，编写函数、类。

使用`template`来声明一个模板，只需要`template<typename/class T>`，其中typename和class可以互换：
```
// template声明创建一个模板
...
```
这样，就创建了一个模板函数，该函数调用时，有两种方法来定义数据类型
- 自动类型推导，直接写实参，自动推导形参类型`mySwap(a1, b1);`，必须推导出一致的数据类型，即a1和b1必须为同样的数据类型
- **推荐**显示指定类型`mySwap<int>(a1, b1);`，直接告诉T=int类型，这种方法用的比较多，这种方法可以发生**隐式类型转换**

**隐式类型转换**存在于普通的函数和显示指定类型的模板函数中，自动类型推导不能进行该转换，指的是实参输入值可以于形参不一样，编译器会自动进行类型转换：
```
void Func(int a, intb);

int a; char c; 
Func(a, c);  // 此时char c会被转为int
```
函数模板是可以发生**重载**的，当普通函数和模板函数的函数名一样时，**优先**调用普通函数，可以使用**空模板参数列表**来强制调用函数模板，重载的模板函数调用和普通重载函数调用是类似的：
```
int a = 10;
int b = 20;
Func1(a, b);   // 优先调用了普通函数
...
```
如果函数模板可以产生更好的匹配，则调用函数模板，这就不多说了，比如定义了一个普通函数，参数都是`int`，调用时我们传的参数都是`char`，这样的话普通函数要发生隐式转换，而模板函数不用，则调用模板函数。

*如果提供了函数模板，最好就不要提供普通函数了。*

*模板也是有局限性的，比如不能代表数组、类、结构，需要使用模板重载，对特定的类型进行具体化操作。* 例如重载一个函数，实现使用类作为模板的数据类型：
`template<> 函数类型 函数名(类 对象， 类 对象)`

*学习模板不是为了写模板，而是为了能够应用STL提供的模板*
...
```
类模板与函数模板有两点差异：
- 类模板没有自动类型推导，只能显示写出模板类型`Person<string, int> p1("Tom", 20);`
- 只有类模板才能在模板参数列表中有默认参数，`template<class NameType, class AgeType = int>`，在调用时可以不用写出默认参数了，**仅限类模板**

类模板和普通类的成员函数创建时机也是不同的，类模板在调用成员函数时才会创建，而普通类从一开始就创建了成员函数。

如何将类模板的对象作为函数参数呢？有三种方法。
- 1.指定传入参数类型。这种方法直接在传入类模板时，指定该模板的参数类型即可。
```
...
```
- 3.整个类进行模板化，将这个模板声明为模板，根据传入的类，自动推断出该模板代表的类：
```
template<class T>
void Func3(T &p)
{
...
当类模板发生继承时，需要注意以下三点：
- 父类是类模板时，子类在声明时需要指定父类的参数类型，否则编译器无法给子类分配内存
```
template<class T>
class Base
{
...
### 类模板分文件编写
因为类模板中的函数在编译时不会立即创建，所以如果单纯的将类模板的声明和定义分文件，则无法识别相应的函数，解决方法有两种：
- 直接引用类模板定义的`.cpp`文件，因为`.cpp`文件引用了声明的头文件，故可以实现
- 将声明的头文件`.h`和定义文件`.cpp`写到一个文件里，一般用`.h`来表示一般头文件，`.hpp`表示**类模板**实现的头文件。